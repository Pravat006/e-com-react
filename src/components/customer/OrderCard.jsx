        ...
        <div className="w-full flex flex-col sm:flex-row items-start sm:items-center sm:justify-between p-4 border-none  custom-glass-5 shadow-sm rounded-lg min-h-[140px] sm:min-h-[120px]">
            ...
            <div className="flex flex-row items-center ">
                ...
                <div className="flex-grow mb-4 sm:mb-0 min-w-0 sm:min-w-[200px]">
                    <p className="text-lg font-medium text-gray-200 truncate text-wrap">{name}</p>
            ...
            <div className="flex flex-col w-full sm:w-72 ml-0 sm:ml-4 flex-shrink-0 gap-y-2">
                ...
                <p className="text-lg font-semibold text-gray-100">{`₹${price.toLocaleString()}`}</p>

                {/* Status Message */}
                <div className="flex items-center justify-between">
                    <p className={`text-base font-medium ${getStatusColor(status)} text-left  `}>
                        {isDelivered && `Delivered on ${statusDate}`}
                        {isCancelled && `Cancelled on ${statusDate}`}
                        {isOnTheWay && `On the way - ${statusDate}`}
                        {!isDelivered && !isCancelled && !isOnTheWay && `${status} on ${statusDate}`}
                    </p>
                    {/* Action Button */}
                    {isDelivered && (
