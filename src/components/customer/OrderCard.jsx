...
const OrderCard = ({ order }) => (
    <div className="w-full flex flex-col sm:flex-row items-start sm:items-center sm:justify-between p-4 bg-white/5 dark:bg-gray-800/10 shadow-sm rounded-lg min-h-[140px] sm:min-h-[120px] border-b-2 border-black dark:border-white backdrop-blur-sm">
        ...
        <div className="flex flex-row items-center">
            ...
            <div className="flex-grow mb-4 sm:mb-0 min-w-0 sm:min-w-[200px]">
                <p className="text-lg font-medium text-gray-900 dark:text-gray-100 truncate text-wrap">
                    {order.items[0].product?.name}
                </p>
                <div className="h-5"> {/* Fixed height container for color */}
                    {/* {color && <p className="text-sm text-gray-600 dark:text-gray-400">Color: {color}</p>} */}
                </div>
            </div>
        </div>
        ...
        <div className="flex flex-col w-full sm:w-72 ml-0 sm:ml-4 flex-shrink-0 gap-y-2">
            ...
            <p className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                {`₹${order.items[0].product?.price.toLocaleString()}`}
            </p>
            ...
            <div className="flex items-center justify-between">
                ...
                {order.status === 'Delivered' && (
                    <button className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 hover:underline text-sm flex items-center focus:outline-none focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400 rounded-md p-1 self-start sm:self-end transition-colors">
                ...
                {order.status === 'Cancelled' && order.cancellationReason && (
                    <p className="text-sm text-gray-600 dark:text-gray-400 text-left sm:text-right leading-tight bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300 rounded p-2 border-l-4 border-red-200 dark:border-red-600">
